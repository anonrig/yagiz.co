...
| [#1192][issue-1192] | Node.js asked for a tagged release that includes the IDNA punycode `xn--` fix (labels that round-trip under ICU but failed to re-parse under 3.4.4, aborting on setter re-parse). That fix has been on `main` since the idna **v0.5.4** sync ([#1147][pr-1147]). |
...
especially the Node.js team for [#1192][issue-1192] and Debian packaging for the
ABI report that sharpened our export/CI story.
...
[issue-1192]: https://github.com/ada-url/ada/issues/1192
...
[pr-1147]: https://github.com/ada-url/ada/pull/1147
